...
class MyKNNClf():
    ...
    def predict(self, X_pred: pd.DataFrame(dtype = 'object')) -> np.array:
        """Model outputs most popular class or highest probability 
        class inversely proportional to rank or distance of each train row."""
        preds = list()
    ...
    def predict_proba(self, X_pred: pd.DataFrame()) -> np.array:
        """Model outputs mean of neighbours or weighted mean of neighbours 
        inversely proportional to rank or distance of each train row."""
        preds = list()
        for idx, row in X_pred.iterrows():
